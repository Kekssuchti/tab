        ...
        if pd.api.types.is_numeric_dtype(vals) and vals.notna().all():
            dimensions.append(
                go.parcoords.Dimension(
                    label=col, values=vals, tickvals=sorted(vals.unique())
                )
            )
        else:
...
def plot_pairwise_heatmap(
    ...
    numeric_params = [
        c for c in param_cols if pd.api.types.is_numeric_dtype(df[c])
    ] or param_cols
    pairs = [
    ...
    if not pairs:
        return go.Figure().update_layout(
            title="Need at least 2 parameters for heatmaps"
        )
    ...
    for px_name, py_name in pairs:
        pivots.append(
            (
                ...
                py_name,
                df.pivot_table(
                    index=py_name, columns=px_name, values=metric_col, aggfunc="mean"
                ),
            )
        )
    ...
    for idx, (px_name, py_name, pivot) in enumerate(pivots):
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                zmin=zmin,
                zmax=zmax,
                hovertemplate=f"{px_name}=%{{x}}<br>{py_name}=%{{y}}<br>"
                f"{metric_col}=%{{z:.4f}}<extra></extra>",
            ),
            row=row,
            col=col,
        )
...
def _print_summary_table(
    df: pd.DataFrame, param_cols: list[str], metric_col: str, top_n: int = 15
):
...
def _html_page_multi(sections: list[tuple[str, list[go.Figure]]], title: str) -> str:
    ...
    for heading, figures in sections:
        parts.append(
            f'<h2 style="margin-top:40px;border-bottom:2px solid #ccc;'
            f'padding-bottom:4px;">{heading}</h2>'
        )
        for fig in figures:
...
def main():
  ...
  uv run python -m src.exploration.cv_analyzer -id 8dd4ef36 -t accuracy -n 20
    ...
    parser.add_argument(
        "-id", type=str, required=True, help="Pipeline ID (MLflow run ID)."
    )
    parser.add_argument(
        "-m",
        type=str,
    ...
    cv_results_dir = (
        Path(config.dir_mlflow_artifacts) / pipeline_id / "artifacts" / "cv_results"
    )
    ...
    for json_path in json_paths:
        try:
            model, figures, _summary = _analyze_model(
                json_path, args.target_metric, args.top_n, plot_names
            )
        except Exception as e:
